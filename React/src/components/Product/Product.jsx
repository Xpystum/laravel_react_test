...
export default function Product(props){
    ...
    function writeRate(){
        for(let i = 0; i < 5; i++){
            ...
            if(product.reviews.average_value >= i + 1)
                rage.push(
                    <li className="rating__list" key={"index_" + i}>
            ...
            <div className="product__card">
                <div className="product__card--thumbnail">
                    <a className="product__card--thumbnail__link display-block" href="product-details.html">
                        <img className="product__card--thumbnail__img product__primary--img" src={ process.env.PUBLIC_URL+"product5.webp"} alt="product-img"/>
                        <img className="product__card--thumbnail__img product__secondary--img" src={ process.env.PUBLIC_URL+"product6.webp"} alt="product-img"/>
                    </a>
                    ...
                    <ul className="product__card--action d-flex align-items-center justify-content-center">
                        <li className="product__card--action__list">
                            <a className="product__card--action__btn" title="Quick View" data-open="modal1">
                ...
                <div className="product__card--content">
                    <ul className="rating product__card--rating d-flex">
                        ...
                        <li>
                            <span className="rating__review--text">({product.reviews.count}) Review rating: 4.3</span>
                        </li>
                    </ul>
                    <h3 className="product__card--title"><a href="product-details.html">{product.title}</a></h3>
                    <div className="product__card--price">
                        <span className="current__price">${product.price}</span>
                        
                        {
                            (product.old_price)?<span className="old__price"> ${product.old_price}</span>:""
                        }
                        
                    </div>
                    <div className="product__card--footer">
                        <button className="product__card--btn primary__btn" type="button" onClick={()=>{props.onAddProductCart(product.id, product.characteristics[0].id)}}>
